...
| Removed file viewer: deleted Dialog/preview, `onPreview` prop, click handlers and hover styles from uploaded file rows in `DocumentItem.jsx`; removed `submissionLogId` prop from `DocumentItem` usage in `PortalPage.jsx` (Spanish law compliance) | ~5 min | 15 min |
| Changed "Optional" badge display text to "If Applicable" in `DocumentItem.jsx` (underlying data unchanged) | ~1 min | 2 min |
