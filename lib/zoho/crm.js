...
export async function listAttachments(module, recordId) {
  // Auxiliary call — must never crash the page that uses it.
  try {
    const token = await getAccessToken();

    const res = await axios.get(
      `${ZOHO_BASE_URL}/crm/v8/${module}/${recordId}/Attachments?fields=File_Name,Created_Time,Modified_Time,Size`,
      {
        headers: { Authorization: `Zoho-oauthtoken ${token}` },
        validateStatus: () => true,
      },
    ...
    console.error(
      "Zoho listAttachments error:",
      res.status,
      JSON.stringify(res.data),
    );
    return [];
  } catch (err) {
    console.error("Zoho listAttachments exception:", err?.message ?? err);
    return [];
  }
}

export async function uploadAttachment(module, recordId, file, filename) {
  const token = await getAccessToken();
...
export async function getNotes(module, recordId) {
  const token = await getAccessToken();
  const res = await axios.get(
    `${ZOHO_BASE_URL}/crm/v8/${module}/${recordId}/Notes?fields=Note_Title,Note_Content,Created_Time,Created_By`,
    {
      headers: { Authorization: `Zoho-oauthtoken ${token}` },
      validateStatus: () => true,
    }
